  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`bg-gradient-to-r from-indigo-600 to-emerald-600 rounded-2xl p-0.5 shadow-md overflow-hidden transition-all duration-500 ease-in-out ${isExpanded ? 'max-h-[1000px]' : 'max-h-[84px]'}`}>
          <div className={`bg-white rounded-[14px] transition-all duration-500 ${isExpanded ? 'p-6' : 'p-4'}`}>
              <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                      <div className={`transition-all duration-500 rounded-xl flex items-center justify-center ${isExpanded ? 'w-10 h-10 bg-indigo-100 text-indigo-600' : 'w-8 h-8 bg-indigo-50 text-indigo-50'}`}>
                          <Sparkles className={`text-indigo-600 ${isExpanded ? 'w-6 h-6 animate-pulse' : 'w-4 h-4'}`} />
                      </div>
                      <div className="flex items-center gap-3">
                          <h3 className={`font-bold text-slate-800 transition-all ${isExpanded ? 'text-lg' : 'text-sm'}`}>AI Daily Briefing</h3>
                          {!isExpanded && (
                  ...
                  <div className="flex items-center gap-2">
                      {briefing && !isBriefingLoading && !briefing.includes("Retrying") && !briefing.includes("quota") && (
                          <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-lg border border-slate-100">
                             <button 
                                ...
                                className={`p-1.5 rounded-md transition-all flex items-center gap-2 text-xs font-bold ${isSpeaking ? 'bg-indigo-600 text-white animate-pulse' : 'text-slate-600 hover:bg-white hover:shadow-sm'}`}
                                title={isSpeaking ? "Stop Speaking" : "Listen to Briefing"}
                             >
                                {isSpeaking ? <Square className="w-3.5 h-3.5" /> : <Volume2 className="w-3.5 h-3.5" />}
                                {isExpanded && <span className="pr-1">{isSpeaking ? 'Stop' : 'Listen'}</span>}
                             </button>
                             <button 
                      ...
                      <button 
                          ...
                          className={`flex items-center gap-2 transition-all font-bold shadow-sm active:scale-95 border ${isExpanded ? 'bg-slate-900 text-white border-slate-900 px-6 py-2.5 rounded-xl text-sm' : 'bg-white text-slate-700 border-slate-200 px-4 py-1.5 rounded-lg text-xs hover:bg-slate-50'}`}
                      >
              ...
              {isExpanded && (
                <div className="mt-6 animate-in fade-in slide-in-from-top-4 duration-500">
                    {isBriefingLoading && !briefing?.includes("Retrying") ? (
                        ...
                        <div className={`rounded-xl p-5 border ${briefing?.includes("quota") ? 'bg-red-50 border-red-100' : 'bg-slate-50 border-slate-100'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm"><div className="flex items-center justify-between mb-4"><h3 className="text-slate-500 font-medium">Active Leads</h3><Circle className="w-5 h-5 text-blue-500" /></div><p className="text-3xl font-bold text-slate-800">{activeLeads}</p></div>
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm"><div className="flex items-center justify-between mb-4"><h3 className="text-slate-500 font-medium">Companies</h3><Building2 className="w-5 h-5 text-orange-500" /></div><p className="text-3xl font-bold text-slate-800">{companiesCount}</p></div>
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm"><div className="flex items-center justify-between mb-4"><h3 className="text-slate-500 font-medium">People</h3><UserIcon className="w-5 h-5 text-indigo-500" /></div><p className="text-3xl font-bold text-slate-800">{peopleCount}</p></div>
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm"><div className="flex items-center justify-between mb-4"><h3 className="text-slate-500 font-medium">Customers</h3><CheckCircle2 className="w-5 h-5 text-emerald-500" /></div><p className="text-3xl font-bold text-slate-800">{closedDeals}</p></div>
      </div>

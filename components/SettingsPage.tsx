...
import { Plus, Edit3, Trash2, Mail, User, Shield, Check, X, Lock, Eye, AlertTriangle, Crown, List as ListIcon, Calendar as CalendarIcon, Type as TypeIcon, Hash, Building2, UserCircle2, FileUp, Loader2 } from 'lucide-react';
import { ImportModal } from './ImportModal';

export const SettingsPage = ({ teamMembers = [], onAddTeamMember, onUpdateTeamMember, onDeleteTeamMember, currentUser, customFields = [], onAddCustomField, onDeleteCustomField, onImportContacts, onClearContacts, isClearingContacts = false, onConnectGmail, onConnectGoogleCalendar, isGmailConnected = false, isCalendarConnected = false }: any) => {
                  ...
                  <div className="border-b border-slate-100 pb-4 flex items-start justify-between gap-4">
                      ...
                      {onClearContacts && (
                          <button
                              ...
                              className="shrink-0 px-3 py-1.5 text-xs font-medium bg-red-50 text-red-600 border border-red-200 rounded-lg hover:bg-red-100 transition-colors flex items-center gap-1.5 disabled:opacity-60 disabled:cursor-not-allowed"
                          >
                              {isClearingContacts ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}
                              {isClearingContacts ? 'Clearing...' : 'Clear All Contacts'}
                          </button>
                      )}
                  </div>
          ...
          {activeTab === 'integrations' && canManageIntegrations && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                 ...
                 {/* 1. Zoom */}
                 <div className="flex items-center justify-between p-4 border border-emerald-200 bg-emerald-50/40 rounded-lg">
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          ...
                          <div className="text-xs text-slate-500">Auto-generates meeting links on booking</div>
                       </div>
                    </div>
                    <span className="px-3 py-1.5 text-xs font-semibold bg-emerald-100 text-emerald-700 rounded-lg border border-emerald-200 flex items-center gap-1.5">

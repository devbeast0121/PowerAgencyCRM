...
import { 
  ...
  Building2, User as UserIcon, Mail, Phone, ChevronRight, Search, 
...
export function App() {
  ...
  // Hoisted helper to fix "missing initAudio" in callbacks
  const initAudio = () => {
  ...
  // Hoisted helper to fix "missing stopLiveCall" in callbacks
  ...
  const handleCompose = (initialData: { to?: string, subject?: string, body?: string, cc?: string, bcc?: string } = {}) => {
      setComposeTo(initialData.to || ''); setComposeSubject(initialData.subject || ''); setComposeBody(initialData.body || ''); setShowCcBcc(!!initialData.cc || !!initialData.bcc); setIsComposeOpen(true); setIsComposeMinimized(false);
  };

  const handleComposeSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      setIsComposeOpen(false);
      const newEmail = { id: `sent-${Date.now()}`, sender: "Me", senderEmail: user?.email || "me@example.com", subject: composeSubject, snippet: composeBody.replace(/<[^>]*>?/gm, '').substring(0, 50), body: composeBody, date: new Date(), isRead: true, isStarred: false, folder: 'sent' };
      ...
      setComposeBody(''); setComposeTo(''); setComposeSubject(''); setShowCcBcc(false);
  };
  ...
  // Live Call Implementation using latest guidelines
  const handleStartLiveCall = async (contactId: string) => {
    const contact = contacts.find(c => c.id === contactId);
    if (!contact) return;

    const allPhones = contact.phones?.map((p: any) => `${p.type}: ${p.value}`).join(', ') || contact.phone || 'None';
    const allEmails = contact.emails?.map((e: any) => `${e.type}: ${e.value}`).join(', ') || contact.email || 'None';
    const contactEmailList = (contact.emails || []).map((e: any) => e.value?.toLowerCase()).filter(Boolean);
    if (contact.email) contactEmailList.push(contact.email.toLowerCase());

    const contactNotes = notes.filter((n: any) => n.contactId === contactId);
    const notesSummary = contactNotes.map(n => `[${n.type || 'Note'} - ${new Date(n.createdAt?.seconds * 1000).toLocaleDateString()}] Content: ${n.content.replace(/<[^>]*>?/gm, '')}${n.rawTranscript ? ` FULL TRANSCRIPT: ${n.rawTranscript}` : ''}`).join('\n---\n');

    const contextString = `Name: ${contact.name}, Company: ${contact.company}, Status: ${contact.status}, Phone: ${allPhones}, Emails: ${allEmails}, Background: ${contact.background}. Notes: ${notesSummary}`;
    ...
    try {
        ...
        // Fix: Use direct initialization from process.env.API_KEY
        ...
        const sessionPromise = ai.live.connect({
            ...
            callbacks: {
                onopen: () => {
                    ...
                    scriptProcessor.onaudioprocess = (e) => {
                        ...
                        // Correct: Solely rely on sessionPromise resolves
            ...
            config: {
                ...
                systemInstruction: `You are a world-class CRM analyst briefing a user about their client: ${contact.name}. Use the following context: ${contextString}`
            }
        });
  ...
  const handleGeminiAction = async (action: string) => {
    ...
    try {
        ...
        if (action === 'custom') prompt = `${geminiPrompt}\n\nText to process:\n${contextText}`;
        else if (action === 'fix') prompt = `Fix grammar and spelling: ${contextText}`;
        else if (action === 'formal') prompt = `Rewrite formal: ${contextText}`;
        else if (action === 'shorten') prompt = `Shorten: ${contextText}`;
        else if (action === 'draft_reply') prompt = `Draft a reply: ${contextText}`;
        else if (action === 'summarize') prompt = `Summarize thread: ${contextText}`;
        ...
        if (generatedText) {
            ...
            if (action === 'draft_reply' || action === 'summarize') {
                const prefix = action === 'summarize' ? '<b>TL;DR Summary:</b> ' : '';
                setComposeBody(prefix + htmlText + '<br /><br />' + composeBody);
            } else {
                setComposeBody(htmlText);
            }
            setIsGeminiOpen(false);
            setGeminiPrompt('');
        }
    } catch (error) {
        alert("Unable to complete request with Gemini.");
    } finally {
  ...
  const handleGenerateContactSummary = async (contactId: string) => {
    ...
    setSummaryText("Harvesting all client data...");
    try {
        ...
        const prompt = `Provide detailed summary for: ${contact.name}. History: ${contactNotes.map(n => `[${n.type}] ${n.content.replace(/<[^>]*>?/gm, '')}`).join('; ')}`;
        // Fix: Use direct initialization from process.env.API_KEY
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const response = await ai.models.generateContent({ model: 'gemini-3-flash-preview', contents: prompt });
        setSummaryText(response.text || "No summary generated.");
    } catch (error) { setSummaryText(`Failed to generate summary.`); } finally { setIsSummaryLoading(false); }
  };
  ...
  const filteredContacts = useMemo(() => {
    ...
    if (advancedFilters.type !== 'All') result = result.filter(c => c.type === advancedFilters.type);
    if (advancedFilters.lifecycleStage !== 'All') result = result.filter(c => c.lifecycleStage === advancedFilters.lifecycleStage);
    if (advancedFilters.groupId !== 'All') result = result.filter(c => c.groups && c.groups.includes(advancedFilters.groupId));
    if (advancedFilters.pipelineId !== 'All') result = result.filter(c => c.pipelineId === advancedFilters.pipelineId);
    return result;
  }, [contacts, searchQuery, filterType, advancedFilters]);
      ...
      <div className="flex h-screen bg-white font-sans text-slate-900 overflow-hidden relative">
            ...
            <div className={`mx-auto h-full ${view === 'email' ? 'max-w-full' : 'max-w-7xl'}`}>
              ...
              {view === 'email' && !selectedContactId && (
                 <EmailPage 
                   ...
                   initialSelectedEmailId={initialSelectedEmailId} 
                   onClearInitialEmailId={() => setInitialSelectedEmailId(null)} 
                   onCompose={handleCompose} 
                   onUpdateEmail={handleUpdateEmail} 
                   onDeleteEmail={handleDeleteEmail} 
                   isGoogleConnected={isGoogleEmailConnected} 
                   onConnectGoogle={handleConnectGoogle} 
                   onNavigateContact={(id: string) => { setSelectedContactId(id); setView('contacts'); }} 
                   onBookMeeting={(contact: any) => { setInitialCalendarBooking({ name: contact.name, email: contact.email }); setView('calendar'); setSelectedContactId(null); }}
                   onSummarize={handleSummarizeText}
                   onOpenAddContact={(data: any) => { setContactModalInitialData(data); setIsContactModalOpen(true); }}
                 />
              )}
              {view === 'groups' && !selectedContactId && (
                  <GroupsPage 
                    contacts={contacts} tagGroups={tagGroups} onGroupClick={(filter: string) => { setFilterType(filter); setView('contacts'); }} onAddNewGroup={() => setIsGroupModalOpen(true)} onEditGroup={handleEditGroup} onDeleteGroup={handleDeleteGroup}
                  />
              )}
              ...
              {view === 'todo' && !selectedContactId && (
                  <TodoPage 
                      user={user} contacts={contacts} onNavigate={(id: string) => { setSelectedContactId(id); setView('contacts'); }} todos={todos} onToggle={handleToggleTodo} onDelete={handleDeleteTodo} onAdd={handleAddTodo}
                  />
              )}
                ...
                <div className="h-full flex flex-col">
                  {!selectedContactId && (
                    <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-6">
                      <div><h2 className="text-2xl font-bold text-slate-800">Contacts</h2><p className="text-slate-500">{filterType === 'All' ? 'All contacts' : (['Leads', 'Customers', 'Not a Fit'].includes(filterType) ? `Viewing ${filterType}` : `Viewing Group: ${filterType}`)}</p></div>
                      <div className="flex flex-col sm:flex-row items-center gap-4 w-full xl:w-auto">
                        <div className="flex p-1 bg-white border border-slate-200 rounded-lg shadow-sm">
                          {['All', 'Leads', 'Customers', 'Not a Fit'].map((f) => (
                            <button key={f} onClick={() => setFilterType(f)} className={`px-3 py-1.5 text-sm font-medium rounded-md transition-all whitespace-nowrap ${filterType === f ? 'bg-slate-100 text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}>
                        ...
                        <div className="flex p-1 bg-white border border-slate-200 rounded-lg shadow-sm">
                          <button onClick={() => setContactsViewMode('list')} className={`p-1.5 rounded-md transition-all ${contactsViewMode === 'list' ? 'bg-slate-100 text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`} title="List View"><List className="w-4 h-4" /></button>
                          <button onClick={() => setContactsViewMode('gallery')} className={`p-1.5 rounded-md transition-all ${contactsViewMode === 'gallery' ? 'bg-slate-100 text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`} title="Gallery View"><LayoutGrid className="w-4 h-4" /></button>
                        </div>
                  ...
                  {selectedContactId && selectedContact ? (
                      <ContactDetail 
                         ...
                         notes={notes.filter(n => n.contactId === selectedContactId)} 
                         emails={emails} 
                         ...
                         onNavigate={(id: string) => { setSelectedContactId(id); setView('contacts'); }} 
                         onViewEmail={(emailId: string) => { setInitialSelectedEmailId(emailId); setView('email'); setSelectedContactId(null); }}
                         tagGroups={tagGroups} 
                         onAddNewGroup={() => setIsGroupModalOpen(true)}
                         user={user} 
                         onGroupClick={handleGroupClick} 
                         onCreateLinkedCompany={handleOpenCreateCompanyModal} 
                         onCreateCompany={handleAddContact}
                         onCompose={handleCompose} 
                         onGenerateSummary={handleGenerateContactSummary}
                         pipelines={pipelines}
                         liveCallProps={liveCallProps}
                      />
                  ) : (
                    view === 'contacts' && (
                      contactsViewMode === 'list' ? (
                          ...
                          <div className="overflow-x-auto">
                            <table className="w-full text-left">
                              <thead className="bg-slate-50 border-b border-slate-200">
                                <tr>
                                  <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider w-32 text-center">Contact Type</th>
                                    ...
                                    <tr key={contact.id} onClick={() => setSelectedContactId(contact.id)} className="hover:bg-slate-50 cursor-pointer transition-colors group">
                                      <td className="px-6 py-4">
                                        <div className="flex justify-center">
                                          {contact.type === 'Company' ? <Building2 className="w-5 h-5 text-orange-500" /> : <UserIcon className="w-5 h-5 text-blue-500" />}
                                        </div>
                                      </td>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                          {filteredContacts.map(contact => (
                            <div key={contact.id} onClick={() => setSelectedContactId(contact.id)} className="group bg-white rounded-xl border border-slate-200 p-5 flex flex-col items-center text-center shadow-sm hover:shadow-md transition-all cursor-pointer relative">
                              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                 <div className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-slate-50 rounded-full"><ChevronRight className="w-4 h-4" /></div>
                              </div>
                              <div className="relative mb-3">
                                 {contact.photo ? (<img src={contact.photo} alt={contact.name} className="w-16 h-16 rounded-full object-cover border-2 border-slate-100 shadow-sm" />) : (<div className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl mb-1 ${contact.type === 'Company' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>{contact.type === 'Company' ? <Building2 className="w-8 h-8" /> : <UserIcon className="w-8 h-8" />}</div>)}
                              </div>
                              <h3 className="font-bold text-slate-900 truncate w-full px-2">{contact.name}</h3>
                              <div className="text-xs text-slate-500 mb-3 h-8 w-full px-2 truncate flex items-center justify-center">{contact.type === 'Company' ? (contact.website || 'No website') : (<>{contact.title && <span className="truncate max-w-[50%]">{contact.title}</span>}{contact.title && contact.company && <span className="mx-1">•</span>}{contact.company && <span className="font-medium text-slate-700 truncate max-w-[50%]">{contact.company}</span>}</>)}</div>
                              <div className="mt-auto flex items-center justify-center gap-2 w-full pt-3 border-t border-slate-100"><div className="ml-auto"><StatusBadge status={contact.status} /></div></div>
                            </div>
                          ))}
                        </div>
                      )
              ...
              {view === 'pipeline' && <Pipeline 
                  contacts={contacts} 
                  setContactId={(id: string) => { setSelectedContactId(id); setView('contacts'); }} 
                  onUpdateStatus={handleUpdateContact} 
                  pipelines={pipelines}
        ...
        {isContactModalOpen && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto">
              ...
              <ContactForm 
                  existingCompanies={contacts.filter(c => c.type === 'Company')} 
                  allContacts={contacts} 
                  tagGroups={tagGroups} 
                  onSubmit={handleAddContact} 
                  onCancel={() => { setIsContactModalOpen(false); setContactModalInitialData(null); setModalCallback(null); }} 
                  onAddNewGroup={() => setIsGroupModalOpen(true)}
        ...
        {isGroupModalOpen && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-[60]">
            <div className="bg-white rounded-lg shadow-xl w-full max-sm p-6">
              <h3 className="text-lg font-bold text-slate-800 mb-4">{editingGroup ? 'Edit Group' : 'Add New Group'}</h3>
              <div className="space-y-4">
                <div><label className="block text-sm font-medium text-slate-700 mb-1">Group Name</label><input className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none" value={newGroupName} onChange={e => setNewGroupName(e.target.value)} /></div>
              </div>
              <div className="flex justify-end gap-2 mt-6"><button onClick={handleCloseGroupModal} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg">Cancel</button><button onClick={handleCreateGroup} className="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">Save</button></div>
            </div>
          </div>
        )}
        {isComposeOpen && (
          <div className={`fixed bottom-0 right-4 md:right-16 bg-white rounded-t-lg shadow-2xl border border-slate-200 z-[80] flex flex-col transition-all duration-300 ${isComposeMinimized ? 'w-64 h-12' : 'w-auto md:w-[600px] h-[80vh] md:h-[600px]'}`}>
              <div className="bg-slate-900 text-white px-4 py-2 rounded-t-lg flex justify-between items-center cursor-pointer shrink-0" onClick={() => setIsComposeMinimized(!isComposeMinimized)}>
                  <span className="font-medium text-sm">New Message</span>
                  <div className="flex items-center gap-2">
                      <button onClick={(e) => { e.stopPropagation(); setIsComposeMinimized(!isComposeMinimized); }} className="hover:bg-white/10 p-1 rounded"><Minimize2 className="w-3.5 h-3.5" /></button>
                      <button onClick={(e) => { e.stopPropagation(); setIsComposeOpen(false); }} className="hover:bg-white/10 p-1 rounded"><X className="w-3.5 h-3.5" /></button>
                  </div>
              </div>
              {!isComposeMinimized && (
                  <form onSubmit={handleComposeSubmit} className="flex-1 flex flex-col overflow-hidden">
                      <input className="px-4 py-2.5 border-b border-slate-100 outline-none text-sm" placeholder="To" value={composeTo} onChange={e => setComposeTo(e.target.value)} />
                      <input className="px-4 py-2.5 border-b border-slate-100 outline-none text-sm font-bold" placeholder="Subject" value={composeSubject} onChange={e => setComposeSubject(e.target.value)} />
                      <div className="flex-1 overflow-hidden flex flex-col min-h-0"><RichTextEditor value={composeBody} onChange={setComposeBody} placeholder="Draft message..." /></div>
                      <div className="p-3 border-t flex justify-between items-center">
                          <button type="submit" className="bg-blue-600 text-white px-6 py-2.5 rounded-full font-bold text-sm shadow-md flex items-center gap-2">Send <Send className="w-3.5 h-3.5" /></button>
                          <button type="button" onClick={() => setIsGeminiOpen(!isGeminiOpen)} className={`p-2.5 rounded-full ${isGeminiOpen ? 'bg-indigo-100 text-indigo-600' : 'text-slate-500'}`}><Sparkles className="w-5 h-5" /></button>
                      </div>
                  </form>
              )}
          </div>
